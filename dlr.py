...
def handle_client(client_socket, remote_host, remote_port):
    ...
    def handle_http(client_socket, remote_socket):
        while True:
            data = client_socket.recv(4096)
            if not data:
                break

            # Handle HTTP request here if needed
            # For simplicity, we'll just forward the data as-is
    ...
    http_thread = threading.Thread(target=handle_http, args=(client_socket, remote_socket))
...
def main():
    ...
    local_port = 4848  # Change to desired local port
